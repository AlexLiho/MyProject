...
var calculatePrice = function () {
    if (!$('select[name="order[product_id]"]').length) {
        return;
    }
    var additionalHtml = "";
    var total = 0;
    var type = $('select[name="order[variant]"]').val();
    var promoCode = $('input[name="promo"]').val();
    var flattype = parseInt($('select[name="order[product_id]"]').val());
    ...
    var windowNum = parseInt($('input[name="order_item[5][quantity]"]').val());
    ...
    var balNum = parseInt($('input[name="order_item[12][quantity]"]').val());
    total += bal * balNum;
    if (balNum) {
        additionalHtml += "<p>Мытье балконов " + balNum + "шт</p>";
    }
    var lustraNum = parseInt($('input[name="order_item[8][quantity]"]').val());
    total += lustra * lustraNum;
    if (lustraNum) {
        additionalHtml += "<p>Мытье люстр " + lustraNum + "шт</p>";
    }
    var holodNum = parseInt($('input[name="order_item[6][quantity]"]').val());
    total += holod * holodNum;
    if (holodNum) {
        additionalHtml += "<p>Мытье холодильника " + holodNum + "шт</p>";
    }
    var duhovNum = parseInt($('input[name="order_item[7][quantity]"]').val());
    total += duhov * duhovNum;
    if (duhovNum) {
        additionalHtml += "<p>Мытье духовки/СВЧ внутри " + duhovNum + "шт</p>";
    }
    var glad1Num = parseInt($('input[name="order_item[9][quantity]"]').val());
    total += glad[1] * glad1Num;
    if (glad1Num) {
        additionalHtml += "<p>Глажка " + glad1Num + "шт</p>";
    }
    var glad2Num = parseInt($('input[name="order_item[10][quantity]"]').val());
    total += glad[2] * glad2Num;
    if (glad2Num) {
        additionalHtml += "<p>Поменять бельё " + glad2Num + "шт</p>";
    }
    var glad3Num = parseInt($('input[name="order_item[11][quantity]"]').val());
    total += glad[3] * glad3Num;
    if (glad3Num) {
        additionalHtml += "Особое поручение " + glad3Num + "шт</p>";
    }
    if ($('input[name="order[get_key]"]').val() === "true") {
        total += getkey;
        additionalHtml += "<p>Заехать за ключами от квартиры</p>";
    }
    if ($('input[name="order[eco]"]').val() === "true") {
        total += total * eco / 100;
...
$().ready(function () {
    ...
    $('.selector > select option').click(function () {
        var value = $(this).attr("value");
        var text = $(this).html();
        $(this).parent().parent().find("option").val(value);
        $(this).parent().slideUp();
